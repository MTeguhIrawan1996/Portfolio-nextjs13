...
import ProjectSkeleton from '@/components/features/projects/common/elements/ProjectSkeleton';
...
const ProjectCardList = () => {
  const { data, isLoading } = useReadAllPortfolio();
      ...
      <div className='grid grid-cols-1 gap-6 p-4 md:grid-cols-2'>
        {isLoading ? (
          ...
          data?.map(({ title, imgUrl, projectLink, tags, techs }, i) => {
            const newArray = STACKS.filter((item) =>
              techs?.includes(item.name)
            );
            return (
